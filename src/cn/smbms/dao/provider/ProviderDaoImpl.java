	...
	public int add(Connection connection, Provider provider)
		...
		if(null != connection){
			String sql = "insert into smbms_provider (proCode,proName,proDesc," +
					"proContact,proPhone,proAddress,proFax,createdBy,creationDate,pidpic) " +
					"values(?,?,?,?,?,?,?,?,?,?)";
			Object[] params = {provider.getProCode(),provider.getProName(),provider.getProDesc(),
								...
								provider.getProFax(),provider.getCreatedBy(),provider.getCreationDate(),provider.getPidpic()};
	...
	public Provider getProviderById(Connection connection, String id)
		...
		if(null != connection){
			...
			if(rs.next()){
				...
				provider.setPidpic(rs.getString("pidpic"));
			}
			BaseDao.closeResource(null, pstm, rs);
		}
		return provider;
